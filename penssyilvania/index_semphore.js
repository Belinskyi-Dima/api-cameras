...
async function getPaSecureToken(id, originUrl) {
  const key = `pa:token:${id}`;

  const cached = await cacheGetRaw(key);
  if (cached !== null) {
    console.log('[PA HIT paTokenCache] :', cached);
    if (cached === DISABLED) return '';
    return cached; // "?token=..."
  }
  console.log('[PA MISS paTokenCache] :', cached);
  ...
  return await withDistributedLock({
    ...
    //   for (const url2 of endpoints) {
        ...
        const r2 = await httpPostText('https://pa.arcadis-ivds.com/api/SecureTokenUri/GetSecureTokenUriBySourceId', token1, 5000);
...
async function getPenssylvania(req, res) {
  ...
  try {
    ...
    if (cached) { 
        console.log('[ PA HIT m3u8 ] cache: ', cacheKey);
        
      res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
      return res.send(cached);
    }
    console.log('[ MISS M3U8 ] cache: ', cacheKey);
    ...
    if (err && err.code === 'ECONNABORTED') {
      return res.status(504).send('Upstream timeout (PA)');
    }
    return res.status(502).send(`Upstream error (PA): ${err.message}`);
  }
}
